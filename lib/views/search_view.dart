...
class _SearchViewState extends State<searchView> {
  ...
  void updateList(String value, BuildContext context) {
    setState(() {
      display_list = Provider.of<UserProvider>(context, listen: false).allUsers.where((element) => element.username!.toLowerCase().contains(value.toLowerCase())).toList();
    });
  }
          ...
          children: [
            ...
            Expanded(
                child: ListView.builder(
                    itemCount: display_list.length > 4 ? 5: display_list.length,
                    itemBuilder: (context, index) => ListTile(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) =>
                              ProfileView(userId: display_list[index].id, isFollowingUser: Provider.of<UserProvider>(context).followingUser(display_list[index].id) == true)
                          ),
                        );
                      },
                      title: Text(display_list[index].username, style: TextStyle(

                      ),
                      ),
                      subtitle: Text(
                        Provider.of<UserProvider>(context).followingList.contains(display_list[index].id) ? "Following" : ""
                      ),
                    ))
            )
          ],
